...
class SucesoPrescriptorDataAccess implements IDataAccess<ISucesoPrescriptor> {
    ...
    get(): Promise<IErrorResponse | Array<ISucesoPrescriptor>> {
     ...
     * @returns 
     */
    async insert(data: ISucesoPrescriptor): Promise<ISucesoPrescriptor | IErrorResponse> {
        throw new Error("Method not implemented.")
     ...
     * @returns 
     */
    async getByGrupoId(idGrupo: BigInt): Promise<IErrorResponse | Array<ISucesoPrescriptor>> {
        // throw new Error("Method not implemented.")
        const queryData  = {
        ...
        let lData: Array<ISucesoPrescriptor | IErrorResponse> = (await this.client.exeQuery(queryData)) as Array<ISucesoPrescriptor | IErrorResponse>
        
        if ( ({ ...lData[0] } as IErrorResponse).error ) return lData[0] as IErrorResponse

        return lData as Array<ISucesoPrescriptor>
    }
}

export default SucesoPrescriptorDataAccess