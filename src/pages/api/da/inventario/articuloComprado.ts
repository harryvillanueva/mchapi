...
import { IResponseGeneral } from "@/api/modelsextra/IResponseGeneral";const handler = nc({
...
.patch(async (req: NextApiRequest , res : NextApiResponse <IResponse | IErrorResponse>)=>{
    ...
    let dataDB : IResponseGeneral | IErrorResponse = await el.añadirProductoComprado(data)
    if ( !dataDB ) {
