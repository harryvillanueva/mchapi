...
const handler = nc(
      ...
      .get(async (req, res: NextApiResponse<IResponse | IErrorResponse>) => { 
            ...
            let dataDB: IApartment | IErrorResponse = await el.getByIdDA(BigInt(parseInt(req.query.id as string)))
            if ( !dataDB ) {
      ...
      //       }
      //       let dataDB: IApartment | IErrorResponse = await el.updateDA(BigInt(parseInt(req.query.id as string)), data)
      //       if ( !dataDB ) {
      //             res.status(404).json({ error: 'data not found' })
      //             return
      //       }
      ...
      //       res.json({ data: dataDB })
      // })  
      ///// añadir cuando este el front terminado /////
      .patch(async (req: NextApiRequest, res: NextApiResponse<IResponse | IErrorResponse>) => { 
            const {idUserLogin, filterState} = UtilInstance.getDataRequest(req)
            let el: ApartmentBusiness = new ApartmentBusiness(idUserLogin, filterState, true)
            ...
            let dataDB: IApartment | IErrorResponse = await el.updateDA_info(BigInt(parseInt(req.query.id as string)), data)
            if ( !dataDB ) {
                  res.status(404).json({ error: 'data not found' })
                  return
            }
