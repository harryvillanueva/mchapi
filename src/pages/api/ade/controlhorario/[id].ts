...
const handler = nc(
    ...
    .get(async (req, res: NextApiResponse<IResponse | IErrorResponse>) => { 
        ...
        let el: ControlHorarioLimpiezaBusiness = new ControlHorarioLimpiezaBusiness(idUserLogin, filterState, false)
        
        let dataDB: IControlHorarioLimpieza | IErrorResponse = await el.getById(BigInt(parseInt(req.query.id as string)))
    ...
    .patch(async (req: NextApiRequest, res: NextApiResponse<IResponse | IErrorResponse>) => { 
        ...
        let dataDB: IControlHorarioLimpieza | IErrorResponse = await el.update(BigInt(parseInt(req.query.id as string)), data)
        if ( !dataDB ) {
                res.status(204).json({ error: 'data not found' })
                return
        }
    ...
    .delete(async (req: NextApiRequest, res: NextApiResponse<IResponse | IErrorResponse>) => { 
        ...
        let dataDB: IControlHorarioLimpieza | IErrorResponse = await el.delete(BigInt(parseInt(req.query.id as string)))
        if ( !dataDB ) {
                res.status(204).json({ error: 'data not found' })
                return
        }
