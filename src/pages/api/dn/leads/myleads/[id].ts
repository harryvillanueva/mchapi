...
const handler = nc(
      ...
      .patch(async (req: NextApiRequest, res: NextApiResponse<IResponse | IErrorResponse>) => { 
            ...
            let estatus = (req.body.estatus && (req.body.estatus == 1 || req.body.estatus == 0)) ? req.body.estatus : 1

            let tipo_accion = req.body.tipo_accion ? req.body.tipo_accion as string : 'Procesar'

            if (tipo_accion === 'Procesar') estatus = 1
            ...
            let dataDB: ILead | IErrorResponse = await el.updateDNCall(BigInt(parseInt(req.query.id as string)), data)
            if ( !dataDB ) {
                  res.status(404).json({ error: 'data not found' })
                  return
            }
