...
import SucesosDnBusiness from '@/api/business/SucesosDnBusiness'
import { ISucesosDn } from '@/api/models/ISucesosDn'
import { ISucesosDnUser } from '@/api/modelsextra/ISucesosDnUser'
...
const handler = nc(
    ...
    .get(async (req, res: NextApiResponse<IResponse | IErrorResponse>) => {
        ...
        let el: SucesosDnBusiness = new SucesosDnBusiness(idUserLogin, filterState, false)
        let dataDB: Array<ISucesosDn> | IErrorResponse = await el.getByUserSuceso(BigInt(parseInt(req.query.id as string) || 0))
        ...
        if ( ( dataDB as Array<ISucesosDn> ).length === 0 ) {
                res.status(404).json({ error: 'data not found' })
                return
        }
