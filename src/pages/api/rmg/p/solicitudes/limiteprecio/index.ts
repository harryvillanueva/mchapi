...
.get(async (req , res : NextApiResponse <IResponse | IErrorResponse>)=>{
    ...
    let el: SolicitudPrecioBLL = new SolicitudPrecioBLL(idUserLogin , filterState , false , {filter : dataFilter})
    let dataDB : Array <ISolicitudPrecio> | IErrorResponse | undefined = undefined
    dataDB = await el.getAllWithPagination()
    ...
    if ( (dataDB as Array<ISolicitudPrecio>).length === 0 ) {
        res.status(204).json({ error: 'data not found' })
        return
    }
