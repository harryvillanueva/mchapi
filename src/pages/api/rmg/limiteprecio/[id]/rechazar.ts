...
const handler = nc(
    ...
    .patch(async (req: NextApiRequest, res: NextApiResponse<IResponse | IErrorResponse>) => { 
        ...
        EmailServiceInstance.changeMailOptions('', 'Solicitud de límite de precio [ RECHAZADA ]', TemplateEmails.getHtmlResponseSolicituLimitePrecio(`${(dataDB as ISolicitudPrecio).piso}`, `${data.limite_precio}`, `${data.porcentaje_limite_precio}`, 'RECHAZADA', `${data.observacion}`))
