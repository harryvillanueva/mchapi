...
const handler = nc(
      ...
      .get(async (req, res: NextApiResponse<IResponse | IErrorResponse>) => {
            ...
            let dataDB: IKey | IErrorResponse = await el.getById(BigInt(parseInt(req.query.id as string)))
            if ( !dataDB ) {
      ...
      .patch(async (req: NextApiRequest, res: NextApiResponse<IResponse | IErrorResponse>) => {
            ...
            let dataDB: IKey | IErrorResponse = await el.update(BigInt(parseInt(req.query.id as string)), data)
            if ( !dataDB ) {
                  res.status(404).json({ error: 'data not found' })
                  return
            }
      ...
      .delete(async (req: NextApiRequest, res: NextApiResponse<IResponse | IErrorResponse>) => {
            const {idUserLogin, filterState} = UtilInstance.getDataRequest(req)
            let el: KeyBusiness = new KeyBusiness(idUserLogin, filterState, false)
            let dataDB: IKey | IErrorResponse = await el.delete(BigInt(parseInt(req.query.id as string)))
            if ( !dataDB ) {
                  res.status(404).json({ error: 'data not found' })
                  return
            }
